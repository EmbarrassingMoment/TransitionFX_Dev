...
## 設計思想
TransitionFXは、**インディーや小規模チームの開発者が、制作中のゲームに後から遷移エフェクトを簡単に追加できる**ことを最優先に設計されています。専任のテクニカルアーティストがいなくても、追加のコードをほとんど書くことなく、ゲームにクオリティの高いトランジションを組み込むことを可能にすることを目標にしました。
...
*   **Sequence Play（シーケンス再生）:**
    `TransitionSequencePreset` データアセットを作成し、`Play Transition Sequence And Wait` ノードを使うと、複数ステップを順番に連続再生できます。
...
*   **OnTransitionProgressChanged:** トランジション再生中、毎Tickイージング適用済みの進捗値（0.0〜1.0）をブロードキャストします。
...
> 以下のプレビューはすべて **Iris** エフェクトで `Mode: Forward` に固定し、Invert フラグの違いのみを確認できるようにしています。

| Invert | 動作 | Preview |
| :--- | :--- | :--- |
| **Off**（デフォルト） | エフェクトの形状が画面を**覆う**方向に動作（内側へ閉じる）。通常のフェードアウト。 | ![Forward Invert Off](docs/images/forward_invert_off.gif) |
| **On** | エフェクトの形状が画面を**開く**方向に動作（外側へ広がる）。マスクが反転。 | ![Forward Invert On](docs/images/forward_invert_on.gif) |

> **Tip:** `Reverse` モードを使わずにフェードインを実現したい場合は `Mode: Forward` + `Invert: True` の組み合わせが使えます。
...
| :--- | :--- | :--- |
| **Fade** | 標準的な不透明度のフェード。シンプルで軽量です。 | ![Fade](docs/images/effect_fade.gif) |
...
| **Corner Wipe** | 指定した画面の角（四隅）を基準点とし、対角線に向かって領域が斜めに拡大していくワイプ。開始する角はパラメータ(0〜3)で自由に選択可能。 | ![Corner Wipe](docs/images/effect_cornerwipe.gif) |
...
| **TV Switch Off** | ブラウン管テレビの電源を切った時のように、画面が上下に潰れて横線になり、最後に中央の点に向かって消滅するレトロなエフェクトです。 | ![TV Switch Off](docs/images/effect_tv_switch_off.gif) |
...
- [x] **トランジションチェイン / シーケンス** `Medium` — `TransitionSequencePreset` と `PlayTransitionSequenceAndWait` を追加し、プリセット連続再生に対応
...
1. `UObject` を継承し `ITransitionEffect` を実装する新しい C++ クラスを作成します。
2. 必須メソッド `Initialize`、`UpdateProgress`、`Cleanup`、`SetInvert`、`SetParameters` を実装します。
3. `TransitionPreset` で `Effect Class` をカスタムクラスに設定し、カスタムマテリアルを割り当てます。

リファレンス実装については [`ITransitionEffect.h`](Plugins/TransitionFX/Source/TransitionFX/Public/ITransitionEffect.h) と [`PostProcessTransitionEffect`](Plugins/TransitionFX/Source/TransitionFX/Public/PostProcessTransitionEffect.h) を参照してください。

## FAQ

### Q: TransitionFX は UEFN（Unreal Editor for Fortnite）で動作しますか？
