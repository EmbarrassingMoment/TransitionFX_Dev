...
*   **プロジェクトタイプ:** **C++ プロジェクト・Blueprint-only プロジェクトの両方**で動作します。通常の使用に C++ のコーディングは不要です。
*   **レンダリング:** PostProcess 対応のレンダリングパイプライン（Deferred または PostProcess 有効の Forward）が必要です。
*   **プラットフォーム:** Windows（DX12 SM6）、Linux（Vulkan SM6）。コンソールおよびモバイルプラットフォームは公式にはテストされていません。SDF ベースのエフェクトは GPU 負荷が中心のため、低スペック環境でのパフォーマンスは変動する可能性があります。
...
| **Heart Iris** | プロシージャルSDFを使用したハート型のアイリスワイプ。 | ![Heart Iris](docs/images/effect_heart_iris.gif) |
...
| **Zoom Wipe** | フェードアウトしながらシーンを内側に歪めてズームする方向性ワイプ。 | ![Zoom Wipe](docs/images/effect_zoom_wipe.gif) |
...
| **Pixelate** | 画面の解像度を徐々に下げていくピクセル化エフェクト。フェードアウトまでモザイクが進行します。 | ![Pixelate](docs/images/effect_pixelate.gif) |
...
よくある質問とトラブルシューティングについては [FAQ](FAQ_JP.md) を参照してください。
