...
*   **Event System:** `OnTransitionStarted`、`OnTransitionHalfway`、`OnTransitionCompleted`デリゲートを使用して、正確なゲームプレイロジックのタイミングを取得できます。
...
*   **HalfwayThreshold:** トランジションが「中間」とみなされるポイント（0.0〜1.0）を定義します（デフォルト：0.5）。
...
*   **OnTransitionHalfway:** 進行状況が`HalfwayThreshold`を超えたときに発火します（レベルロードやカメラ切り替えに便利です）。
