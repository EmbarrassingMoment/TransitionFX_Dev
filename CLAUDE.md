...
| `UWidgetTransitionEffect` | `Public/WidgetTransitionEffect.h` | Concrete implementation for the widget layer: full-viewport Slate `SImage` with a UI-domain dynamic material, added via `AddViewportWidgetContent` at `UTransitionPreset::WidgetZOrder`; covers UMG/Slate UI |
| `TransitionFXMaterialUtils` | `Public/TransitionFXMaterialUtils.h` | `ApplyParameters()` — shared scalar/vector/texture override logic used by both effect classes |
...
└── Content/
    ├── Data/          ← 40 DA_*.uasset presets (30 effects + DA_FadeToBlack + 9 DA_Widget_*) + Sequence/ sample
    ├── Materials/     ← M_Transition_*.uasset master materials + instances
    │   └── Widget/    ← M_Widget_*.uasset UI-domain variants + Instances/MI_Widget_* (generated by DevMaterialTools/Tools/build_widget_material.py)
...
- **PostProcess limitation**: `UPostProcessTransitionEffect` does not cover UMG/Slate UI layers. `UWidgetTransitionEffect` (full-viewport Slate `SImage` via `AddViewportWidgetContent`, preset `WidgetZOrder`) does, but only for effects with a canonical `SceneTexture → Lerp` final stage; scene-resampling effects (Pixelate) and the A/B-swapped Slice master are deliberately unsupported on the widget layer.
