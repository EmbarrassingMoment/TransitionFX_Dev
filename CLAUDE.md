...
- The `L_ShowCase` level (showcases 28 of the 29 transition effects; only `DA_LinearWipe` is currently not referenced by the level. The preset list lives in the level Blueprint's `PostProcess` variable — a soft-object array, kept in alphabetical order)
